...
module lobattoHex_mod
    use iso_fortran_env
    use integrator_mod
    implicit none
    
    type, extends(Integrator) :: LobattoIntegrator3DHex
    contains
        procedure :: setPointsWeights => lobattoHex_setPointsWeights
    end type

    interface LobattoIntegrator3DHex
        procedure :: lobattoHex_ctor
    end interface
    
contains

    function lobattoHex_ctor(numPts) result(this)
        implicit none 

        type(LobattoIntegrator3DHex)   :: this
        ...
        this % name = "LobattoIntegrator3DHex"
    ...
    subroutine lobattoHex_setPointsWeights(this)
        implicit none
    
        class(LobattoIntegrator3DHex), intent(inout) :: this
        ...
        if (this % isInit()) then
            ...
            call sroXIT(" >>>>>> lobattoHex_setPointsWeights <<<<<< Object not initialised ")
        end if
